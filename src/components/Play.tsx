...
const Play = () => {
  const [gameState, currentBetTotal] = useGameplaySelector((gp) => [
    gp.gameState,
    gp.currentBetTotal,
  ]);
  const dispatch = useDispatch();
  const canPlay =
    gameState === GameState.BETTING && D(currentBetTotal).greaterThan(0);
  ...
  return (
    <div
      onClick={canPlay ? startPlay : canReset ? reset : undefined}
      className={`${classes.play} ${canPlay || canReset ? classes.active : ""}`}
    >
