...
const Messages = () => {
  const [
    ...
    currentWinnings,
    playerDidWin,
  ] = useGameplaySelector((gp) => [
    gp.gameState,
    gp.currentBet,
    gp.computerChoice,
    gp.roundWinningChoice,
    gp.currentWinnings,
    gp.playerDidWin,
  ]);
    ...
    case GameState.PLAYING:
      message = (
        <div className={classes.playing}>
          <div ref={computerChoiceDivRef} className={classes.computerChoice}>
            <span ref={computerChoiceSpanRef}></span>
          </div>
          <div className={classes.vs}>VS</div>
          <div className={classes.playerChoices}>
            {Object.entries(currentBet)
              .filter(([key, value]) => D(value).gt(0))
              .map(([key, value]) => (
    ...
    case GameState.WINNINGS:
      message = (
        <div className={classes.winnings}>
          ...
          >{`${roundWinningChoice?.toUpperCase()} WON`}</div>
          {playerDidWin && currentWinnings && D(currentWinnings).gt(0) && (
            <div className={`${classes.amountWon}`}>
              <span>YOU WIN: </span>
              <span ref={currentWinningsRef}>
                {/* {currentWinnings.toString()} */}
              </span>
            </div>
          )}
        </div>
    ...
    if (
      ...
      gsap.to(computerChoiceSpanRef.current, {
        ...
        onComplete: () => {
          ...
          computerChoiceDivRef.current!.classList.add(
            "animate__animated",
            "animate__pulse",
          );
          sleep(2500).then(() => {
  ...
  useLayoutEffect(() => {
    if (
      gameState === GameState.WINNINGS &&
      playerDidWin &&
      currentWinningsRef.current
    ) {
      ...
      gsap.to(winningsDisplay, {
        ...
        onUpdate: () => {
          currentWinningsRef.current!.textContent = D(
            winningsDisplay.current,
          ).toFixed(2);
        },
        onComplete: () => {
          currentWinningsRef.current!.textContent =
            D(currentWinnings).toFixed(2);
          currentWinningsRef.current!.classList.add(
            "animate__animated",
            "animate__pulse",
          );
        },
  ...
  return (
    <div className={`${classes.messages} ${classes[gameState]}`}>{message}</div>
  );
};
